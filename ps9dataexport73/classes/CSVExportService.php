...
class CSVExportService
{
    ...
    {
        // Campos según documentación oficial de PrestaShop
        $headers = array(
            'ID',
            'Active (0/1)',
            'Name *',
        ...
        foreach ($products as $product) {
            ...
            // ID
            $row[] = $this->cleanInt($product['id_product'], 1);
            ...
            // Price per unit - IMPORTANTE: puede causar error si es NULL o inválido
            $unitPrice = $product['unit_price_ratio'] ?? 0;
            // Si unit_price_ratio es 0 o NULL, dejarlo en blanco para evitar errores
            $row[] = ($unitPrice > 0) ? $this->cleanDecimal($unitPrice) : '';
    ...
    public function exportCategories($outputFile)
    {
        ...
        $headers = array(
            'Active (0/1)',
            'Name *',
            'Parent category', // Puede ser nombre o ID
            'Root category (0/1)',
            'Description',
            'Meta title',
            'Meta keywords',
